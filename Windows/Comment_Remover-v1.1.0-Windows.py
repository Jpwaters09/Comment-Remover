################################################
## Comment Remover v1.1.0 - Windows           ##
## Language: Python                           ##
## Author: Jacob Waters                       ##
...
class Window(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("Comment Remover v1.1.0")
        self.setGeometry(0, 0, 600, 400)
        self.setFixedSize(600, 400)
        icon = extract_icon("Comment_Remover-v1.1.0-Windows.exe", IconSize.LARGE)
    ...
    def selectLang(self):
        ...
        self.codeLangSelector.addItems(["Coding Language", "Python", "JavaScript", "Java", "C", "C++", "C#", "Lua", "Asm"])
        ...
        for idx, line in enumerate(lines):
            ...
            while i < len(line):
                if line[i] == "'" and not is_char_escaped(line, i) and not inside_double_quote and not inside_backtick:
                    ...
                    if line[i:i+2] == "//" and not (inside_single_quote or inside_double_quote or inside_backtick):
                        line = line[:i].rstrip()
                        break
                
                elif commentH == True:
                    if line[i] == "#" and not (inside_single_quote or inside_double_quote or inside_backtick):
                        line = line[:i].rstrip()
                        break
    ...
    def showFileDialog(self):
        ...
        if self.lang == "Java":
            file_path, _ = QFileDialog.getOpenFileName(self, "Open File", '', "Java Files (*.java)")
        ...
        if self.lang == "C#":
            file_path, _ = QFileDialog.getOpenFileName(self, "Open File", '', "C# Files (*.cs)")

        if self.lang == "Lua":
            file_path, _ = QFileDialog.getOpenFileName(self, "Open File", '', "Lua Files (*.lua)")

        if self.lang == "Asm":
            file_path, _ = QFileDialog.getOpenFileName(self, "Open File", '', "Asm Files (*.asm *.s)")
